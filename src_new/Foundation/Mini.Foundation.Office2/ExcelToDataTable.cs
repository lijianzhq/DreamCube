...
namespace Mini.Foundation.Office
{
    public class ExcelToDataTable : IExcelToDataTable
    {
        ...
        {
            MyValidation.ArgumentNotNullOrEmpty(excelFilePath, "excelFilePath");
            if (!File.Exists(excelFilePath))
                throw new FileNotFoundException(String.Format("excel file [{0}] does not exsit!", excelFilePath));
        ...
        public virtual DataTable GetDataTable(String sheetName, Boolean isFirstRowColumnName = false)
        {
            ThrowIfNotInitialized();
