...
namespace LY.MQCS.Plugin.Datagrid
{
    public class DataTransfer : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var rspParam = new ResponseParam()
            {
                OpResult = true,
                OpData = null,
            };
            ...
            catch (Exception ex)
            {
                ...
                rspParam.Message = ex.Message;
                if (ex.InnerException != null)
                    rspParam.Message += ex.InnerException.Message;
            }
            finally
            {
        ...
        /// <returns></returns>
        protected void LoadData(RequestParam rqParam, ref ResponseParam rspParam)
        {
        ...
        /// <param name="rspParam">响应参数</param>
        /// <returns></returns>
        protected void LoadColumnEditData(RequestParam rqParam, ref ResponseParam rspParam)
        {
        ...
        /// <param name="rspParam">响应参数</param>
        /// <returns></returns>
        protected void InitDatagrid(RequestParam rqParam, ref ResponseParam rspParam)
        {
            //在此处写入您的处理程序实现。
            var grid = GetGrid(rqParam);
        ...
        protected EasyUIDataGrid GetGrid(RequestParam rqParam)
        {
            ...
            {
                SQL = "select * from V1_ALL_QUES where rownum<=10",
